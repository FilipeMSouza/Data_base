...
class consertoModel{
    ...
    public function update($mecanico, $placa, $datarevisao, $valorrevisao, $descricao, $clientecpf){
        $sql = "UPDATE public.conserto SET mecanico = '$mecanico', placa = '$placa', $datarevisao = 'datarevisao', $valorrevisao = 'valorrevisao', $descricao = 'descricao', $clientecpf = 'clientecpf' WHERE placa = 'placa' "; 
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
    }
    ...
    public function select_by_id($id){
        $stmt = $this->pdo->query("SELECT mecanico, placa, datarevisao, valorrevisao, descricao, clientecpf FROM public.conserto WHERE clientecpf = '$id' ");
